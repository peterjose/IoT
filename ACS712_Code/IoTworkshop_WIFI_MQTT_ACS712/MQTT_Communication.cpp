...
// Setup a feed called 'CurrentSensor' for publishing.
// Notice MQTT paths for AIO follow the form: <username>/feeds/<feedname>
Adafruit_MQTT_Publish currentSensorData = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/CurrentSensor");
...
/**
   @Brief   Function to trigger the Server Update
   @Param   None
   @Return  None
*/
...
/*
   @Brief   Function to initialise the MQTT communication
   @Param   None
   @Return  None
*/
...
/*
   @Brief   Function to handle the MQTT server task
   @Param   None
   @Return  None
*/
  ...
  if (ServerUpdateFlag == true)
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      if (MQTT_connect())
      {
        ...
        // Now we can publish
        if ( !(currentSensorData.publish(GetSensorData())))
        {
          DEBUG_PRINT_LN(F("Failed"));
        }
        else
        {
          DEBUG_PRINT_LN(F("Sensor Data send status OK!"));
        }
        delay(100);
      }
